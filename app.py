...
"""
            ...
            st.info(f"اكتملت المقاعد الأساسية. أنت في صدارة الاحتياط رقم ({lb.get('wait_pos', 1)}). سيتم إشعارك وتصعيدك فور اعتذار أي لاعب.")
...
# ==========================================
# 8. لوحة الإدارة والتحليلات المالية المتقدمة
# ==========================================
with st.expander("⚙️ لوحة الإدارة والتحليلات المالية", expanded=False):
    ...
    if pin_input:
        ...
        if is_valid:
            ...
            with get_db() as conn:
                ...
                # 1. إحصائيات السيولة والتدفق النقدي لتمرين اليوم
                cur.execute("SELECT COUNT(*) FROM bookings WHERE session_day=? AND status='confirmed'", (db_session_key,))
                today_confirmed = cur.fetchone()[0]

                cur.execute("SELECT COUNT(*) FROM bookings WHERE session_day=? AND status='confirmed' AND payment_status='paid'", (db_session_key,))
                today_paid_count = cur.fetchone()[0]

                cur.execute("SELECT COUNT(*) FROM bookings WHERE session_day=? AND status='confirmed' AND payment_status='pending'", (db_session_key,))
                today_pending_count = cur.fetchone()[0]

                cash_in_hand = today_paid_count * 65
                pending_cash = today_pending_count * 65
                court_rent_est = 260 # تقدير تكلفة الكورت لـ 90 دقيقة
                net_profit_est = (today_confirmed * 65) - court_rent_est

                # 2. مؤشرات الأداء المالي اللحظي
                st.markdown("##### 💵 التدفق النقدي واقتصاديات الوحدة (تمرين اليوم):")
                m1, m2, m3 = st.columns(3)
                m1.metric("المحصل فعلياً", f"{cash_in_hand} ر.س", f"{today_paid_count} لاعبين")
                m2.metric("مبالغ معلقة", f"{pending_cash} ر.س", f"{today_pending_count} انتظار دفع", delta_color="inverse")
                m3.metric("صافي الربح التقديري", f"{net_profit_est} ر.س", f"نقطة التعادل: {court_rent_est} ر.س")

                # 3. حساب السعة التشغيلية وإدارة العائد (Yield Management)
                st.markdown("---")
                st.markdown("##### 📈 كفاءة السعة التشغيلية وإدارة العائد:")
                load_factor = round((today_confirmed / 6.0) * 100, 1)
                cur.execute("SELECT COUNT(*) FROM bookings WHERE session_day=? AND status='waitlist'", (db_session_key,))
                waitlist_count = cur.fetchone()[0]

                y1, y2 = st.columns(2)
                y1.metric("معامل الامتلاء (Load Factor)", f"{load_factor}%", f"{today_confirmed}/6 مقاعد")
                y2.metric("الطلب الإضافي (الاحتياط)", f"{waitlist_count} لاعبين", "فرصة توسع" if waitlist_count >= 4 else "سعة متوازنة")

                if waitlist_count >= 4:
                    st.warning("💡 **توصية تشغيلية:** يوجد أكثر من 4 لاعبين في الاحتياط! يمكنك حجز كورت ثانٍ وتحويلهم لجلسة موازية بربح مؤكد.")
                elif load_factor < 67:
                    st.info("💡 **توصية تسويقية:** المقاعد المتبقية أكثر من 2؛ يفضل إرسال تذكير في قروب الواتساب لرفع الامتلاء قبل موعد الإلغاء المجاني للملعب.")

                # 4. التزامات الولاء المؤجلة (IFRS 15)
                st.markdown("---")
                st.markdown("##### 🎁 محاسبة برنامج الولاء والالتزامات المؤجلة:")
                cur.execute("""
                    SELECT phone, name, COUNT(DISTINCT session_day) as played
                    FROM bookings 
                    WHERE status='confirmed'
                    GROUP BY phone, name
                """)
                loyalty_players = cur.fetchall()

                total_liability = 0
                eligible_free = []
                for lp in loyalty_players:
                    pts = lp[2] % 7
                    total_liability += (pts * 9.29)
                    if pts == 6:
                        eligible_free.append(f"{lp[1]} ({lp[0]})")

                l1, l2 = st.columns(2)
                l1.metric("إجمالي الالتزامات المؤجلة", f"{round(total_liability, 1)} ر.س", "مخصصة للمقاعد المجانية")
                l2.metric("المستحقون للمجاني القادم", f"{len(eligible_free)} لاعبين")

                if eligible_free:
                    st.success("🌟 **لاعبون يستحقون المقعد المجاني في حجزهم القادم:** " + " • ".join(eligible_free))

                # 5. التعديل السريع لحالة الدفع
                st.markdown("---")
                st.markdown("##### ⚡ تأكيد الدفع السريع للاعبي اليوم:")
                cur.execute("SELECT id, name, phone, payment_status FROM bookings WHERE session_day=? AND status='confirmed'", (db_session_key,))
                players_to_pay = cur.fetchall()

                for p_id, p_name, p_phone, p_pay in players_to_pay:
                    col_p1, col_p2 = st.columns([3, 1])
                    with col_p1:
                        st.caption(f"🎾 **{p_name}** ({p_phone}) — الحالة: `{p_pay}`")
                    with col_p2:
                        if p_pay == 'pending':
                            if st.button("تأكيد السداد ✅", key=f"pay_{p_id}", use_container_width=True):
                                cur.execute("UPDATE bookings SET payment_status='paid' WHERE id=?", (p_id,))
                                conn.commit()
                                st.rerun()

                # 6. تصدير البيانات الشامل
                st.markdown("---")
                cur.execute("""
            ...
            if raw_data:
                ...
                st.download_button(
                    "📥 تصدير السجل المالي المنسق (Excel/CSV)",
                    csv_buf.getvalue().encode('utf-8-sig'),
                    f"padel_financial_export_{datetime.now().strftime('%Y%m%d')}.csv",
                    "text/csv",
