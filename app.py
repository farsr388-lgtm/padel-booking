...
# ==============================================================================
# 1. إعداد الصفحة وتنسيق الهواتف (CSS)
# ==============================================================================
st.set_page_config(
    page_title="بادل 99",
    page_icon="🎾",
...
try:
    ...
    st.error("تعذر الاتصال بقاعدة البيانات حالياً.")
    st.stop()

# ==============================================================================
# 3. محرك توقيت الجلسة التلقائي
# ==============================================================================
def resolve_next_session() -> tuple[str, str]:
    ksa_tz = timezone(timedelta(hours=3))
    current_dt = datetime.now(ksa_tz)
    ...
    days_to_add, day_name = weekday_offsets.get(current_dt.weekday(), (0, "الأحد"))
    
    # الانتقال لليوم التالي بعد الساعة 10:30 مساءً
    if days_to_add == 0 and current_dt.hour >= 22 and current_dt.minute >= 30:
        next_dt = current_dt + timedelta(days=1)
        days_to_add, day_name = weekday_offsets.get(next_dt.weekday(), (0, "الأحد"))
        days_to_add += 1
        
    target_date = current_dt + timedelta(days=days_to_add)
...
# استرجاع المشتركين المؤكدين
try:
    confirmed_players = supabase.table("bookings") \
        .select("name, level") \
...
confirmed_players = confirmed_players[:COURT_CAPACITY]
...
if confirmed_players:
    ...
    st.markdown(f'<div class="roster-box">👥 <b>المؤكدين بالملعب:</b> {" • ".join(sanitized_names)}</div>', unsafe_allow_html=True)

# ==============================================================================
# 5. شاشة ما بعد الحجز (مع العداد الحي بالـ JavaScript)
# ==============================================================================
if "booked" in st.session_state:
    b = st.session_state["booked"]
    target_epoch_ms = b.get("expire_timestamp", 0)
    
    # كود العداد بالـ JavaScript المربوط بالوقت الثابت
    timer_script = f"""
    ...
    (function() {{
        ...
        function updateTimer() {{
            ...
            if (distance <= 0) {{
                ...
                timerBox.innerHTML = "⚠️ انتهت مهلة الـ 15 دقيقة! يرجى إرسال الإيصال بالواتساب فوراً للتحقق من عدم إلغاء المقعد.";
                return;
            }}
    ...
    with st.form("quick_booking_form", clear_on_submit=True):
        ...
        if btn_submit and not hp:
            ...
            if len(clean_name) < 2:
                ...
                try:
                    # فحص الشواغر لمنع السباق والتسجيل الزائد
                    live_bookings = supabase.table("bookings") \
                        .select("id, phone") \
                        .eq("session_day", db_session_key) \
                        .eq("status", "confirmed") \
                        .execute().data or []
                    
                    if any(item["phone"] == clean_phone for item in live_bookings):
                        st.warning("أنت مسجل بالفعل في هذا التمرين!")
                    elif len(live_bookings) >= COURT_CAPACITY:
                        st.error("عذراً، اكتملت المقاعد للتو!")
                    else:
                        ...
                        # احتساب وقت انتهاء الـ 15 دقيقة بالمللي ثانية لتمريره للـ JS
                        expire_time = datetime.now(timezone.utc) + timedelta(minutes=15)
                        expire_ms = int(expire_time.timestamp() * 1000)
